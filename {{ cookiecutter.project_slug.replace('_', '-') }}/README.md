...
A skeleton deck ships in [`slides.md`](slides.md) with a pre-rendered
[`slides.pdf`](slides.pdf) alongside it — cover, whoami, agenda, one section
divider, one content slide, and a closing slide. Fill in your own content in
`slides.md`. `make build` writes fresh output to `build/slides.pdf`.
...
slides.pdf                 # pre-rendered skeleton (regenerated by `make build`)
themes/
